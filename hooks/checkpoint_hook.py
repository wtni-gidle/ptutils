...
class CheckpointHook(HookBase):
    ...
    def after_iter(self) -> None:
        """在每个 iteration 结束后调用，用于保存检查点。"""
        if self.trainer.args.train_by_epoch:
            return
    ...
    def after_epoch(self) -> None:
        """在每个 epoch 结束后调用，用于保存检查点。"""
        if not self.trainer.args.train_by_epoch:
            return
    ...
    def _delete_oldest_checkpoint(self, checkpoint_name):
        """删除最旧的检查点，以保持最新的最多 max_to_keep 个检查点。

        参数:
            checkpoint_name (str): 要添加到最新检查点列表的检查点名称。
        """
        if self._max_to_keep is not None:
            ...
            if len(self._recent_checkpoints) > self._max_to_keep:
                ...
                file_path = os.path.join(self.trainer.args.ckpt_dir, file_name)
                if os.path.exists(file_path):
                    os.remove(file_path)
                    logger.info(f"Deleted the oldest checkpoint file: {file_path}")
