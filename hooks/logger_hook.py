...
class LoggerHook(HookBase):
    ...
    **The hook has the lowest priority (level 10)**.
    ...
    def after_iter(self) -> None:
        if self.trainer.args.train_by_epoch and self.every_n_inner_iters(self._period):
            self._write_console()
            self._write_tensorboard()
        if not self.trainer.args.train_by_epoch and self.every_n_iters(self._period):
            self._write_console()
            self._write_tensorboard()